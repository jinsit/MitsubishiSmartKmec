...
public class DM_DM01_R02P extends Dialog implements OnClickListener,
		OnItemSelectedListener {

	protected DM_DM01_R02P(Context context, DM_DM01_R00_ITEM03 item) {
		super(context);
	...
	DM_DM01_R00_ITEM03 item;
	private TextView btn_dm_update,btn_dm_delete;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dm_dm01_r02p);
		this.isFirst = true;
		activityInit();
	}
	...
	protected void activityInit() {
		...
		// /title 위젯
		tv01_popTitle.setText("특별O/T 정보");
		// //////////////////
	...
	void dataSetting(){
		...
		tv_dm_bldgNm.setText(item.getBldgNm());
		//스피너셋팅

		//
		et_dm_Rmk.setText(item.getOtRemark());
		tv_dm_carNo.setText(item.getCarNo());
		tv_dm_otTmFr.setText(item.getOtWorkDt()+ " " +item.getOtTmFr());
		tv_dm_otTmTo.setText(item.getOtWorkDt() + " " +item.getOtTmTo());

		otNo = item.getOtNo();
		csEmpId = item.getCsEmpId();
		bldgNo = item.getBldgNo();
		carNo = item.getCarNo();
		otWorkDt = item.getOtWorkDt();
	...
	protected void getInstances() {
		...
		// //////////////////
		btn_dm_update = (TextView) findViewById(R.id.btn_dm_update);
		btn_dm_delete = (TextView) findViewById(R.id.btn_dm_delete);
		...
		//계획일 경우에만 이벤트를 먹이고 수정,삭제도 가능토록

		if(item.getRepSt().equals("0")){
			setEvents();
		}else{
			btn_dm_update.setVisibility(View.GONE);
			btn_dm_delete.setVisibility(View.GONE);
		}
	...
	protected void setEvents() {
		...
		btn_dm_update.setOnClickListener(this);
		btn_dm_delete.setOnClickListener(this);
	...
	public class selectWorkType extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("bagicWorkTime")) {
				...
				DM_DM01_R02P.this.sp_dm_otWorkCd.setAdapter(adapter01);

				int selection = 0;
				if(isFirst){
					for(int i=0;i<itemList01.size();i++){
						if(item.getOtWorkCd().equals(itemList01.get(i).getWorkCd())){
							selection = i;
							break;
						}
					}
				...
				currentSelectedItem = itemList01.get(selection);
				sp_dm_otWorkCd.setSelection(selection);
		...
		protected String doInBackground(String... params) {
			...
			if (params[0].equals("bagicWorkTime")) {
				try {
					...
					List<NameValuePair> arguments = new ArrayList<NameValuePair>();
					arguments.add(new BasicNameValuePair("stTp", stTp)); //I: 입력, U : 수정, D 삭제
					arguments.add(new BasicNameValuePair("csEmpId", csEmpId));
					arguments.add(new BasicNameValuePair("otWorkDt", otWorkDt));
					arguments.add(new BasicNameValuePair("otNo", otNo));
					...
					arguments.add(new BasicNameValuePair("usrId",
							DM_DM01_R02P.this.commonSession.getEmpId()));
					returnJson01 = http.getPost(param_url_01, arguments, true);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("bagicWorkTime")) {
				...
				if (resultFg.equals("1")) {
					AlertView.showAlert("OT 수정 성공 했습니다.", context, new android.content.DialogInterface.OnDismissListener() {
						@Override
						public void onDismiss(DialogInterface dialog) {
							// TODO Auto-generated method stub
							DM_DM01_R02P.this.dismiss();
						}
					});
				} else {
		...
		sBldg.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (no.equals("") || no == null) {
					...
					DM_DM01_R02P.this.setBldgNo(sBldg.getBldgNo());
					DM_DM01_R02P.this.setBldgName(sBldg.getBldgNm());
				}
			}
		...
		elev.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if(no.equals("") || no == null){
					...
					DM_DM01_R02P.this.setCarNo(elev.getReturnStr());
				}
			}
		});
	}
	...
	private void progress(Boolean isActivated) {
		if (isActivated) {
			DM_DM01_R02P.this.progress = android.app.ProgressDialog.show(
					context, "알림", "OT 등록 중입니다.");
		} else {
			DM_DM01_R02P.this.progress.dismiss();
		}
	}
	...
	public void setBldgName(String bldgName) {
		if (bldgName.equals("") || bldgName == null) {
			...
			DM_DM01_R02P.this.tv_dm_bldgNm.setText(bldgName);
			setCarNo("");
		}
	}
	...
	public void setCarNo(String carNo) {
		this.carNo = carNo;
		if (carNo == "" || carNo == null) {
			DM_DM01_R02P.this.tv_dm_carNo.setText(DM_DM01_R02P.this.context
