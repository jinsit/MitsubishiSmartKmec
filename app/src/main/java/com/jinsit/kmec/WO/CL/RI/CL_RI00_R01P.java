...
public class CL_RI00_R01P extends Dialog implements android.view.View.OnClickListener , OnDismissListener{

	public CL_RI00_R01P(Context context, String bldgNo, String refContrNo, String workDt, String jobNo, String clientId, String custSign,Activity activity) {
		super(context);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cl_ri00_r01p);
		activityInit();
	}

	protected void activityInit() {
		getInstances();
		tv01_popTitle.setText("고객 승인");
	...
	private class FtpSignDataUpLoadAsync extends
		...
		protected String doInBackground(String... params) {
			...
			if (params[0].equals("bagicWorkTime")) {
				...
				arguments.add(new BasicNameValuePair("folderToAccess", AbsoluteFilePath.FTP_FOLDER_WORI_PATH));
	...
	public class selectItemImage extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (params[0].equals("bagicWorkTime")) {
				try {
					sign = EasyDownLoad.getImage(AbsoluteFilePath.FTP_FOLDER_WORI_PATH ,  custSign, commonSession.getEmpId());
				} catch (Exception e) {
					e.printStackTrace();
				}
				return params[0];
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("bagicWorkTime")) {
				try {
					...
					img_fr_sign.setVisibility(View.VISIBLE);
					img_fr_sign.setImageBitmap(sign);
				}
				catch(Exception ex){
					AlertView.showAlert( "이미지 불러오기를 실패했습니다.", context);
	...
	private void progress(Boolean isActivated) {
		if (isActivated) {
			CL_RI00_R01P.this.progress = android.app.ProgressDialog.show(
					context, "알림", "조회 중입니다.");
		} else {
			CL_RI00_R01P.this.progress.dismiss();
		}
	}
