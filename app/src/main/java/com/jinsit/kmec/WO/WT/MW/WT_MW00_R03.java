 ...
 * 회의교육 은 NFCTag가 가능해야 하므로 따로 뺀다.
 * @author 원성민
 *
 */
public class WT_MW00_R03 extends Activity implements OnClickListener{
	...
	private void progress(Boolean isActivated) {
		if (isActivated) {
			WT_MW00_R03.this.progress = android.app.ProgressDialog.show(
					context, "알림", "조회 중입니다.");
		} else {
			WT_MW00_R03.this.progress.dismiss();
		}
	}
	...
	private class WorkTargetDetailAsync extends AsyncTask<Void, String, Void> {
		...
		protected void onPostExecute(Void result) {
			WT_MW00_R03.this.ProgressDialog.dismiss();
			try {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			WT_MW00_R03.this.ProgressDialog = android.app.ProgressDialog.show(
					WT_MW00_R03.this, "작업대상", "작업대상목록 불러오는중");
		}
				...
				try {
					...
					if (!isError) {
						...
						if (rtn.equals("0")) {
							...
							// 사무실 태그
							progress(false);
							if(workTargetData.getWORK_NM().equals("사무정리")){
								if(workTargetData.getST().equals("계획")){
									timeRegistration(JOB_ACT_START);
								...
								showAlert("알림. 사무실 NFC TAG입니다.");	
							}
							
						} else if (rtn.equals("2")) {
							// 회의실 태그
							progress(false);
							if(workTargetData.getWORK_NM().equals("회의,교육")){
							...
							// 빌딩테그
							progress(false);
							showAlert("알림. 빌딩 NFC TAG로는 회의를 진행할 수 없습니다.");
						}
